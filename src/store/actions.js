...
import {
  ...
  searchKeywords,
  getShoppingRecord,
  cancelShoppingRecord,
  deleteShoppingRecord,
  deleteAllShoppingRecord,
  createReceiveInfo,
  getReceiveInfo,
  updateReceiveInfo,
  delteReceiveInfo,
...
export default {
  ...
  // 获取收货地址信息
  async reqReceiveInfo({
      commit
    },
    user_id) {
    const result = await getReceiveInfo(user_id);
    if (result.success_code === 200) {
      commit(RECEIVE_INFO, {
        receiveInfo: result.message
      });
    }
  },
  ...
  async reqShoppingRecord({
    ...
    const result = await getShoppingRecord(user_id);
    if (result.success_code === 200) {
      commit(SHOPPING_RECORD, {
        shoppingrecord: result.message
      })
    }
  },

  // 取消订单
  async celShoppingRecord({
    commit
  }, id) {
  ...
  // 删除单条订单数据
  async delShoppingRecord({
    commit
  }, {
    id
  }) {
    const result = await deleteShoppingRecord(id);
    if (result.success_code === 200) {
      commit(DEL_SINGLE_RECORD, {
        shoppingrecord
      })
    }
  },
  ...
  // 删除所有订单数据
  async delAllShoppingRecord({
    commit
  }, {
    user_id
  }) {
    const result = await deleteAllShoppingRecord(user_id);
    if (result.success_code === 200) {
      commit(DEL_ALL_RECORD)
    }
  },
