  ...
  <div class="header_nav">
    <div>
      ...
      <div
        ...
        <span
          class="address">{{selectedOptions[2] || city || '长沙市'}}</span>
        <svg
          viewBox="0 0 32 32"
    ...
    <ul>
      <li><a
          @click.prevent="goOrderCenter">订单中心</a>
      </li>
      <li v-if="!userInfo.id">
        <router-link
          to="/login">您好,请登录
      ...
      <li v-else>
        <a
          v-if="userInfo.user_nickname">您好,{{ userInfo.user_nickname }}</a>
        <a
          v-else>您好,{{ userInfo.user_name | nameFormat }}</a>
        <a
          @click="headerLogout">退出登录</a>
      </li>
      <li
      ...
      <li><a
          @click.prevent="goSelfCenter">个人中心</a>
      </li>
      <li><a
          @click.prevent="goShopCar">我的购物车</a>
      </li>
      <li><a
          @click.prevent="goAdmin">管理员通道</a>
      </li>
      <li><a>联系客服</a></li>
      <li><a>网站导航</a></li>
    </ul>
...
export default {
  ...
  methods: {
    ...
    goSelfCenter(){
      if(this.userInfo.id){
        this.$router.replace('/selfcenter');
      }else{
        MessageBox({
          type:'info',
          message:"请先登录!",
